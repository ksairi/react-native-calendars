...
export default class Agenda extends Component<AgendaProps, State> {
  ...
  renderStaticHeader = (collapseViewAnimation) => {
    ...
    const {customText, month} = staticHeaderOnCollapseView;
  ...
  const customHeaderStyle = {top: 10, marginTop:0, position:"absolute", width:"100%"};
  const mainViewStyle = {...this.style.container};


    return (
      <View testID={testID} onLayout={this.onLayout} style={[mainViewStyle]}>
        ...
        <View style={customHeaderStyle}>{this.renderStaticHeader(collapseViewAnimation)}</View>
